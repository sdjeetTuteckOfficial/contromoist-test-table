...
const ParentComponent = () => {
  ...
  console.log(selectedRowIds,"bhdsjvd");
    ...
    return data.map((item) => {
      ...
      if ( receivedQuantity !== 0 && rejectedQuantity === 0 && 
       receivedQuantity !== item.qty) {
        errors.push("Please enter the remaining quantity as the rejected quantity.");
      }
  ...
  const handleDataChange1 = (rowIndex, columnId, value, id) => {
  ...
  //   const qty = updatedData[rowIndex].qty;
  //   let updatedValue = parseFloat(value);
  //   updatedValue = Math.min(updatedValue, qty);
  //   updatedData[rowIndex][columnId] = updatedValue;
 
  //   // Automatically adjust the other field to match total quantity if overflows
  //   if (columnId === 'received_quantity') {
  //     updatedData[rowIndex].rejected_quantity = Math.max(qty - updatedValue, 0);
  //   } else if (columnId === 'rejected_quantity') {
  //     updatedData[rowIndex].received_quantity = Math.max(qty - updatedValue, 0);
  //   }
